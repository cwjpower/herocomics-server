...
function lps_api_drm( $user_id, $book_id, $device_id = NULL ) {
	...
	/**
	 *  DRM 적용시 서버 로그 : notice  ---
Fri Dec 22 17:19:19.433071 2017] [php7:notice] [pid 5824] [client 114.31.116.193:44304] PHP Notice:  A non well formed numeric value encountered in /var/www/booktalk/includes/lib/pclzip.lib.php on line 1845, referer: http://booktalk.world/mobile/cart/pay.php
[Fri Dec 22 17:19:20.799727 2017] [php7:notice] [pid 5824] [client 114.31.116.193:44304] PHP Notice:  A non well formed numeric value encountered in /var/www/booktalk/includes/lib/pclzip.lib.php on line 1845, referer: http://booktalk.world/mobile/cart/pay.php
	 *
	 */
	...
	// $url = 'https://v2.opensdrm.net/issuelicense/providers/' . $provider_id . '/licenses';
	$url = 'https://api.opensdrm.net/issuelicense/providers/' . $provider_id . '/licenses';
	...
	//$target['type'] = 'device';
	$target['type'] = 'user';                   // 사용자 인증으로 변경
	$target['user_id'] = $user_login;

	//todo:check 사용자 인증으로 변경한다면 type = 'internal' ?
	//todo:check device 관련 정보는 제외함
	//todo: 디바이스 관련 정보 제외 - 사용자 인증 문제 관련
	 //$target['device_id']['type'] = 'external';

	//$target['device_id']['id'] = $device_id;	// 디바이스 인증

	// $target['device_id']['id'] = '';	// 사용자 인증
	...
	if ( !empty($zip_lists) ) {
		foreach ( $zip_lists as $key => $val ) {
			if ( stripos($val['filename'], 'license.lcpl') !== false ) {
				// META-INF/license.lcpl 파일 압축 해제
