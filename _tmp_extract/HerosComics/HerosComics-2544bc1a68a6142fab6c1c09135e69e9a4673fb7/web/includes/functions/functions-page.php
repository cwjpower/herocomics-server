...
function lps_add_banner() {
	...
	$stmt->bind_param( 'sssssssissi', $banner_title, $banner_url, $banner_target, $hide_or_show,
						$file_path, $file_url, $file_name, $banner_order, 
						$banner_from, $banner_to, $user_id
	);
	$stmt->execute();
...
function lps_add_curation() {
	...
	";
	$stmt = $wdb->prepare( $query );
	$stmt->bind_param( 'ssiiisi', $curation_title, $cover_img, $curation_order, $curation_status,
								$curator_level, $curation_meta, $user_id
			);
	$stmt->execute();
	return $wdb->insert_id;
}
...
function lps_edit_curation() {
	...
	";
	$stmt = $wdb->prepare( $query );
	$stmt->bind_param( 'sisi', $curation_title, $curation_status, $curation_meta, $curation_id );
	return $stmt->execute();
}
