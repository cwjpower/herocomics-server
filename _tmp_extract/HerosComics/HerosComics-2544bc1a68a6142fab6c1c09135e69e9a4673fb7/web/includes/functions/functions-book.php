...
function lps_add_book() {
	...
	//error_log("[KOSTA TEST - TC3 - Add book info and create community - START]");
	...
	$stmt->bind_param( 'ssssiiisssssiiisssiii', $book_title, $author, $publisher, $isbn,
						$normal_price, $discount_rate, $sale_price, $cover_img, $epub_path, $epub_name,
						$period_from, $period_to, $category_first, $category_second, $category_third,
						$is_pkg, $is_free, $upload_type, $book_status, $user_id, $publisher_id
	);
	...
	if (!empty($_POST['file_path_epub'][0]) && is_file($_POST['file_path_epub'][0])) {
		...
		if (rename($tmp_path, $new_path)) {
			...
			";
			$stmt = $wdb->prepare( $query );
			$stmt->bind_param( 'ssi', $file_path, $file_name, $book_id );
			$stmt->execute();
		}
	}
	...
	//error_log("[KOSTA TEST - TC3 - Add book info and create community - END]");
...
// 	$user_id = wps_get_current_user_id();
	...
	";
	$stmt = $wdb->prepare( $query );
	$stmt->bind_param( 'ssssiisssssiiisssi', $book_title, $author, $publisher, $isbn,
			$normal_price, $sale_price, $cover_img, $epub_path, $epub_name,
			$period_from, $period_to, $category_first, $category_second, $category_third,
			$is_pkg, $is_free, $upload_type, $book_id
	);
	$stmt->execute();
		...
		if (stripos($_POST['file_path_epub'][0], '/tmp/') !== false) {	// 새로운 epub file 업로드 시
			...
			if (!empty($_POST['file_path_epub'][0]) && is_file($_POST['file_path_epub'][0])) {
				...
				if (rename($tmp_path, $new_path)) {
					...
					";
					$stmt = $wdb->prepare( $query );
					$stmt->bind_param( 'ssii', $file_path, $file_name, $book_status, $book_id );
					$stmt->execute();
				}
			}
		} // .if
...
function lps_get_read_book_data_by_user( $user_id, $book_id ) {
	...
	error_log('lps_get_read_book_data_by_user  $user_id ==========  ' . $user_id);
	error_log('lps_get_read_book_data_by_user  $book_id ==========  ' . $book_id);
	...
	$stmt->bind_param('ii', $user_id, $book_id);
	$stmt->execute();
	error_log('lps_get_read_book_data_by_user end ');
	return $wdb->get_row($stmt);
}
