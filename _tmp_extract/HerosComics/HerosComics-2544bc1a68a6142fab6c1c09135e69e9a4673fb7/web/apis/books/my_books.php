...
$user_id = $_GET['uid'];
error_log('API : 201 USER ID ==============='. $user_id);
...
if (!empty($rows)) {
	...
	foreach ($rows as $key => $val) {
		...
		$rows[$key]['epub_url'] = $epub_url;
		$rows[$key]['epub_name'] = $epub['file_name'];
		unset($rows[$key]['epub']);
	}
	// error_log("after foreach === ");
} else{
	error_log('API : 201 empty book list ===============');
}
