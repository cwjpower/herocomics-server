...
if ( !empty($period_from) && !empty($period_to) ) {
	$period_to_append = $period_to . ' 23:59:59';
	$sql .= " AND l.created_dt BETWEEN ? AND ?";
	array_push( $sparam, $period_from, $period_to_append );
}
			...
			<div class="content-wrapper">
				...
				<div class="content-header">
					...
					<ol class="breadcrumb">
						...
						<li>프로모션</li>
						<li class="active"><b>포인트 사용내역</b></li>
					</ol>
				</div>
				...
				<div class="content body">
					...
					if ( !empty($rows) ) {
						...
						foreach ( $rows as $key => $val ) {
							$point_used = $val['point_used'];
							$point_total = $val['point_total'];	// 각 거래시점 별 잔여 포인트
							$total_point = $val['total_point'];	// 회원의 잔여 포인트 합
							$created_dt = $val['created_dt'];
							$cash_comment = unserialize($val['cash_comment']);
								$order_id = $cash_comment['order_id'];
								$purpose = empty($order_id) ? '' : '<a href="../salestool/order_view.php?id=' . $order_id .'">책 구입</a>';
					...
					?>
									<tr>
										...
										<td>사용</td>
										<td class="text-light-blue"><?php echo number_format($point_used) ?></td>
