...
if ( empty($_GET['id']) ) {
	lps_js_back( '쿠폰 아이디가 존재하지 않습니다.' );
}

$coupon_id = $_GET['id'];
$cp_row = lps_get_coupon_by_id( $coupon_id );

$coupon_name = $cp_row['coupon_name'];
$coupon_type = $cp_row['coupon_type'];
$coupon_desc = $cp_row['coupon_desc'];
$period_from = $cp_row['period_from'];
$period_to = $cp_row['period_to'];
$discount_type = $cp_row['discount_type'];
$discount_amount = $cp_row['discount_amount'];
...
// $related_publisher = $cp_row['related_publisher'];
								...
								<table class="table table-bordered ls-table">
									...
									<tbody>
										...
										<tr>
											...
											<td>
												<label><input type="radio" name="coupon_type" value="item" <?php echo !strcmp($coupon_type, 'item') ? 'checked' : ''; ?>> 개별 책</label> &nbsp; &nbsp;
												<label><input type="radio" name="coupon_type" value="cart" <?php echo !strcmp($coupon_type, 'cart') ? 'checked' : ''; ?>> 장바구니</label>
											</td>
										</tr>
										 -->
										<tr>
											...
											<td>
												<input type="text" name="coupon_name" id="coupon_name" class="form-control" required value="<?php echo $coupon_name ?>">
											</td>
										</tr>
										<tr>
											<td class="item-label">쿠폰 설명</td>
											<td>
												<textarea id="coupon_desc" name="coupon_desc" class="form-control"><?php echo $coupon_desc ?></textarea>
											</td>
										</tr>
										<!-- 
										<tr>
...
// 												$selected = $user_id == $related_publisher ? 'selected' : '';
										?>
													<option value="<?php echo $user_id ?>" <?php echo $selected ?>><?php echo $user_name ?> (<?php echo $display_name ?>)</option>
										<?php
// 											}
// 										}
										?>
										...
										<tr>
											...
											<td>
												<!-- <label><input type="radio" name="discount_type" id="dt-amount" value="amount" <?php echo !strcmp($discount_type, 'amount') ? 'checked' : ''; ?>> 할인금액</label> &nbsp; &nbsp; -->
												<label><input type="radio" name="discount_type" id="dt-rate" value="rate" <?php echo !strcmp($discount_type, 'rate') ? 'checked' : ''; ?>> 할인율</label>
											</td>
										</tr>
										<tr id="dt-amount-pane" <?php echo !strcmp($discount_type, 'amount') ? '' : 'class="hide"'; ?>>
											<td class="item-label">할인 금액 *</td>
											<td>
												<div class="input-group">
													<input type="text" name="discount_amount" id="discount_amount" class="form-control price" value="<?php echo $discount_amount ?>">
													<div class="input-group-addon">원</div>
												</div>
											</td>
										</tr>
										<tr id="dt-rate-pane" <?php echo !strcmp($discount_type, 'rate') ? '' : 'class="hide"'; ?>>
											<td class="item-label">할인율 *</td>
											<td>
												<div class="input-group">
													<input type="text" name="discount_rate" id="discount_rate" class="form-control price" value="<?php echo $discount_rate ?>">
										...
										<tr>
											...
											<td>
												<div class="input-group">
													...
													<input type="text" name="item_price_min" id="item_price_min" class="form-control price" value="<?php echo $item_price_min ?>">
													<div class="input-group-addon">원 이상일때만 사용할 수 있습니다</div>
												</div>
												<div class="input-group" style="margin-top: 8px;">
													<div class="input-group-addon">최대 할인 가능 금액</div> 
													<input type="text" name="item_price_max" id="item_price_max" class="form-control price" <?php echo !strcmp($discount_type, 'rate') ? '' : 'disabled'; ?> value="<?php echo $item_price_max ?>">
