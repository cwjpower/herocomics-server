...
if ( empty($_FILES['profile_bg']['tmp_name']) ) {
	$code = 602;
...
if ( stripos($_FILES['profile_bg']['type'], 'image') === false ) {
	$code = 603;
	$msg = '이미지 파일이 아닙니다.';
	$json = compact('code', 'msg');
	exit( json_encode($json) );
...
$file_ext = strtolower(pathinfo( $_FILES['profile_bg']['name'], PATHINFO_EXTENSION ));
...
$result = move_uploaded_file( $_FILES['profile_bg']['tmp_name'], $upload_path );
...
if ( $result ) {
	...
	// 	$file_name = $_FILES['profile_bg']['name'];
	...
	$user_profile_bg = UPLOAD_URL . '/user_avatar/' . $ym . '/' . $thumb_name;

	rename($thumb_path, $user_profile_path);

	unlink($file_path);

	wps_update_user_meta($uid, 'wps_user_profile_bg', $user_profile_bg);
...
$json = compact( 'code', 'msg', 'user_profile_bg' );
